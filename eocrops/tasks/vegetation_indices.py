...
class BiophysicalIndices:
    def __init__(self, B03, B04, B05, B06, B07, B8A, B11, B12, viewZenithMean, sunZenithAngles, viewAzimuthMean,
                 sunAzimuthAngles):
    ...
    def apply_normalize(self):
        ...
        self.sunZen_norm = self._normalize(funccos(self.sunZenithAngles * degToRad), 0.342022871159208,
                                           0.936206429175402)
...
class VegetationIndicesS2(EOTask):
    ...
    def get_vegetation_indices(self):
        ...
        if self.biophysical:
            biopysicial_parameters = BiophysicalIndices(self.B03, self.B04, self.B05, self.B06, self.B07, self.B8A,
                                                        self.B11, self.B12,
                                                        self.viewZenithMean, self.sunZenithAngles, self.viewAzimuthMean,
